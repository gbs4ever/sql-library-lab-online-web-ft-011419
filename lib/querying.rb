...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM Series JOIN Characters ON series.id = characters.series_id WHERE characters.species = 'human' GROUP BY characters.species ORDER BY COUNT(species) DESC LIMIT 1;"
end
def select_character_names_and_number_of_books_they_are_in
  "SELECT Characters.name , FROM  Characters JOIN ON  "
end
